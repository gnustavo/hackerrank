...
sub expected {
    ...
    for my $x (0 .. $n - 1) {
        ...
        if ($chars eq 'WW' || $chars eq 'BB') {
            ...
            $prob->badd($lexpected > $rexpected ? $lexpected : $rexpected);
        } elsif ($chars eq 'WB') {
            $prob->badd(expected($n - 1, $k - 1, $lballs));
        } elsif ($chars eq 'BW') {
            $prob->badd(expected($n - 1, $k - 1, $rballs));
        }

    }

    my $result = $prob->copy()->badd($picks)->bdiv($n);
